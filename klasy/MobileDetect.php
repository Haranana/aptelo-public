...
class MobileDetect
{
    ...
    public function getHttpHeader($header)
    {
        ...
        if (strpos((string)$header, '_') === false) {
    ...
    public function setCfHeaders($cfHeaders = null) {
        ...
        foreach ($cfHeaders as $key => $value) {
            if (substr(strtolower((string)$key), 0, 16) === 'http_cloudfront_') {
    ...
    public function setUserAgent($userAgent = null)
    {
        ...
        if (false === empty($userAgent)) {
            ...
            if (!empty($this->userAgent)) {
                return $this->userAgent = trim((string)$this->userAgent);
            }
        }
            ...
            if (isset($this->httpHeaders[$mobileHeader])) {
                if (is_array($matchType['matches'])) {
                    foreach ($matchType['matches'] as $_match) {
                        if (strpos((string)$this->httpHeaders[$mobileHeader], $_match) !== false) {
                            return true;
                        }
                    }
    ...
    public function __call($name, $arguments)
    {
        ...
        if (substr((string)$name, 0, 2) !== 'is') {
            throw new BadMethodCallException("No such method exists: $name");
        }

        $this->setDetectionType(self::DETECTION_TYPE_MOBILE);

        $key = substr((string)$name, 2);
    ...
    {
        $ver = str_replace(array('_', ' ', '/'), '.', (string)$ver);
        $arrVer = explode('.', (string)$ver, 2);

        if (isset($arrVer[1])) {
            $arrVer[1] = @str_replace('.', '', (string)$arrVer[1]); // @todo: treat strings versions.
        }

        return (float) implode('.', (array)$arrVer);
    }
