...
class SimpleImage {
  ...
  public function fromDataUri($uri) {
    ...
    preg_match('/^data:(.*?);/', (string)$uri, $matches);
    if (!count($matches)) {
    ...
    switch($this->mimeType) {
      ...
      case 'image/jpeg':
        ...
        if (function_exists('exif_read_data')) {
          $this->exif = @exif_read_data("data://image/jpeg;base64," . base64_encode((string)$file));
        }
        break;
    }
  ...
  public function fromString($string) {
    return $this->fromFile('data://;base64,' . base64_encode((string)$string));
  }
  ...
  public function toDataUri($mimeType = null, $quality = 100) {
    ...
    return 'data:' . $image['mimeType'] . ';base64,' . base64_encode((string)$image['data']);
  }
  ...
  public function overlay($overlay, $anchor = 'center', $opacity = 1, $xOffset = 0, $yOffset = 0, $calculateOffsetFromEdge = false) {
    ...
    if (strpos((string)$anchor, 'top') !== false) {
      $y = $yOffset;
    } elseif (strpos((string)$anchor, 'bottom') !== false) {
      $y = $spaceY + ($calculateOffsetFromEdge ? -$yOffset : $yOffset);
    }

    // Determine if left|right
    if (strpos((string)$anchor, 'left') !== false) {
      $x = $xOffset;
    } elseif (strpos((string)$anchor, 'right') !== false) {
  ...
  public function text($text, $options, &$boundary = null) {
    ...
    if ($calculateOffsetFromEdge == true) {
      if (strpos((string)$anchor, 'bottom') !== false) $yOffset *= -1;
      if (strpos((string)$anchor, 'right') !== false) $xOffset *= -1;
    }
    ...
    if ($baselineAlign == true) {
      ...
      if (strpos((string)$anchor, 'bottom') !== false) {
        $yOffset -= $boxFull[1];
      } elseif (strpos((string)$anchor, 'top') !== false) {
  ...
  public function textBox($text, $options) {
    ...
    if ($align != 'justify') {
      foreach ($lines as $key => $line) {
        if ( $align == 'top' ) $line = trim((string)$line); // If is justify = 'center'
    ...
    // Justify
      ...
      foreach ($lines as $keyLine => $line) {
        ...
        $spaces = 0;
        if (preg_match("/^\s+/", (string)$line, $match)) {
          // Count spaces
          $spaces = strlen((string)$match[0]);
  ...
  */
  ...
  public static function normalizeColor($color) {
    ...
    if (is_string($color) && array_key_exists(strtolower((string)$color), $cssColors)) {
      $color = $cssColors[strtolower((string)$color)];
    }
