...
if ($IloscProduktow > 0) { 
    ...
    echo '<div class="ListingWierszeKontener">';
    ...
    while ($info = $sql->fetch_assoc()) {
        ...
        echo '<div id="prd-' . rand(1,1000) . '-' . $info['products_id'] . '" class="Wiersz LiniaDolna' . (($Produkt->zakupy['mozliwe_kupowanie'] == 'tak' || $Produkt->zakupy['pokaz_koszyk'] == 'tak') ? '' : ' ProduktBezZakupu') . (($GLOBALS['koszykKlienta']->SprawdzCzyDodanyDoKoszyka($Produkt->info['id'])) ? ' ProduktDodanyDoKoszyka' : '') . '">';
            ...
            //
            echo '<div class="ProdCena" style="width:calc(100% - ' . ((int)SZEROKOSC_OBRAZEK_MALY + 50) . 'px)">';
                ...
                if ( !empty($Produkt->info['data_dostepnosci']) ) {
                     //
                     echo '<div class="cl"></div><div class="DataDostepnosci">{__TLUMACZ:DOSTEPNY_OD_DNIA} <b>' . $Produkt->info['data_dostepnosci'] . '</b></div>';
                     //
                }        
