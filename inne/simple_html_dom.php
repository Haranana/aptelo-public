	...
	$defaultSpanText = DEFAULT_SPAN_TEXT)
{
	...
	if (empty($contents) || strlen((string)$contents) > $maxLen) { return false; }
	...
	$defaultSpanText = DEFAULT_SPAN_TEXT)
{
	...
	if (empty($str) || strlen((string)$str) > MAX_FILE_SIZE) {
		$dom->clear();
		return false;
	}
...
class simple_html_dom_node
{
	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
	{
		...
		foreach($nodes as $node) {
			...
			if ($pass && $id !== '' && isset($node->attr['id'])) {
				...
				$node_id = explode(' ', (string)trim((string)$node->attr['id']))[0];
			...
			if ($pass && $class !== '' && is_array($class) && !empty($class)) {
				if (isset($node->attr['class'])) {
					$node_classes = explode(' ', (string)$node->attr['class']);
	...
	protected function match($exp, $pattern, $value, $case_sensitivity)
	{
		...
		switch ($exp) {
			...
			case '~=':
				 ...
				 */
				return in_array($pattern, explode(' ', (string)trim((string)$value)), true);
		}
		return false;
	}
	...
	protected function parse_selector($selector_string)
	{
		...
		preg_match_all(
			$pattern,
			trim((string)$selector_string) . ' ', // Add final ' ' as pseudo separator
			$matches,
			PREG_SET_ORDER
		);
		...
		foreach ($matches as $m) {
			...
			if($m[4] !== '') {
				...
				foreach($attributes as $att) {
					// Skip empty matches
					if(trim((string)$att[0]) === '') { continue; }

					$inverted = (isset($att[1][0]) && $att[1][0] === '!');
					$m[4][] = array(
						$inverted ? substr((string)$att[1], 1) : $att[1], // Name
			...
			// Sanitize Separator
			if ($m[5] !== '' && trim((string)$m[5]) === '') { // Descendant Separator
				$m[5] = ' ';
	...
	function convert_text($text)
	{
		...
		if ($this->dom) {
			$sourceCharset = strtoupper((string)$this->dom->_charset);
		...
		if ($targetCharset === 'UTF-8') {
			if (substr((string)$converted_text, 0, 3) === "\xef\xbb\xbf") {
				$converted_text = substr((string)$converted_text, 3);
			}

			if (substr((string)$converted_text, -3) === "\xef\xbb\xbf") {
				$converted_text = substr((string)$converted_text, 0, -3);
			}
		}
	...
	function get_display_size()
	{
		...
		if (isset($this->attr['style'])) {
			...
			if (isset($attributes['width']) && $width == -1) {
				...
				if (strtolower(substr((string)$attributes['width'], -2)) === 'px') {
					$proposed_width = substr((string)$attributes['width'], 0, -2);
			...
			if (isset($attributes['height']) && $height == -1) {
				// check that the last two characters are px (pixels)
				if (strtolower(substr((string)$attributes['height'], -2)) == 'px') {
					$proposed_height = substr((string)$attributes['height'], 0, -2);
					// Now make sure that it's an integer and not something stupid.
...
class simple_html_dom
{
	 ...
	 *
	 * _Note_: Using this variable is more efficient than calling `strlen((string)$doc)`
	 *
	 * @var int
	 * */
	public $size;
		...
		$options = 0)
	{
		...
		// strip out the \r \n's if we are told to.
		if ($stripRN) {
			$this->doc = str_replace("\r", ' ', (string)$this->doc);
			$this->doc = str_replace("\n", ' ', (string)$this->doc);

			// set the length of content since we have changed it.
			$this->size = strlen((string)$this->doc);
		}
	...
	protected function read_tag()
	{
		...
		if ($this->char === '/') {
			...
			if (($pos = strpos((string)$tag, ' ')) !== false) {
				$tag = substr((string)$tag, 0, $pos);
			}
		...
		do {
			...
			if ($this->doc[$this->pos - 1] == '<') {
				...
				$node->attr = array();
				$node->_[HDOM_INFO_END] = 0;
				$node->_[HDOM_INFO_TEXT] = substr((string)
					$this->doc,
					$begin_tag_pos,
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		// html whitespace.
		$node->attr[$name] = str_replace("\r", '', (string)$node->attr[$name]);
		$node->attr[$name] = str_replace("\n", '', (string)$node->attr[$name]);
		...
		if ($name === 'class') {
			$node->attr[$name] = trim((string)$node->attr[$name]);
		}
	}
	...
	protected function copy_skip($chars)
	{
		...
		$this->char = ($this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
		if ($len === 0) { return ''; }
		return substr((string)$this->doc, $pos, $len);
	}
	...
	protected function copy_until_char($char)
	{
		...
		if (($pos = strpos((string)$this->doc, (string)$char, $this->pos)) === false) {
			$ret = substr((string)$this->doc, $this->pos, $this->size - $this->pos);
			$this->char = null;
			$this->pos = $this->size;
			return $ret;
		}
		...
		$this->pos = $pos;
		return substr((string)$this->doc, $pos_old, $pos - $pos_old);
	}
	...
	protected function remove_noise($pattern, $remove_tag = false)
	{
		...
		for ($i = $count - 1; $i > -1; --$i) {
			...
			$this->doc = substr_replace($this->doc, $key, $matches[$i][$idx][1], strlen((string)$matches[$i][$idx][0]));
		}
	...
	function restore_noise($text)
	{
		...
		while (($pos = strpos((string)$text, '___noise___')) !== false) {
			...
			if (strlen((string)$text) > $pos + 15) {
				...
				if (isset($this->noise[$key])) {
					$text = substr((string)$text, 0, $pos)
					. $this->noise[$key]
					. substr((string)$text, $pos + 16);
				} else {
	...
	function search_noise($text)
	{
		...
		foreach($this->noise as $noiseElement) {
			if (strpos((string)$noiseElement, (string)$text) !== false) {
				return $noiseElement;
			}
		}
	}
