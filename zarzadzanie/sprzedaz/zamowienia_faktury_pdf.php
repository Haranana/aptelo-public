...
if ($prot->wyswietlStrone) {
  ...
  if ( isset($_POST['format']) && $_POST['format'] == 'html' ) {
      ...
      if ( !empty($_POST['data_od']) && !empty($_POST['data_do']) ) {
           //
           header('Content-disposition: attachment; filename=zestawienie_faktur_' . $filtr->process($_POST['data_od']) . '_' . $filtr->process($_POST['data_do']) . '.html');
           //
        } else { 
           //
           header('Content-disposition: attachment; filename=zestawienie_faktur_' . $filtr->process($_POST['data_wydruku_mc']) . '_' . $filtr->process($_POST['data_wydruku_rok']) . '.html');
           //
      }
      ...
      if ( !empty($_POST['data_od']) && !empty($_POST['data_do']) ) {
           //
           $tytul = 'Zestawienie faktur - ' . $filtr->process($_POST['data_od']) . '-' . $filtr->process($_POST['data_do']);
           //
        } else { 
           //
           $tytul = 'Zestawienie faktur - ' . $_POST['data_wydruku_mc'] . '-' . $_POST['data_wydruku_rok'];
           //
      }   
  ...
  if ( isset($_POST['format']) && $_POST['format'] == 'csv' ) {
      ...
      $text = FakturaCsv::WydrukCsv($filtr->process($_POST['data_wydruku_mc']).'-'.$filtr->process($_POST['data_wydruku_rok']), '2', '', $filtr->process($_POST['data_od']), $filtr->process($_POST['data_do']));
      ...
      if ( !empty($_POST['data_od']) && !empty($_POST['data_do']) ) {
           //
           header('Content-disposition: attachment; filename=zestawienie_faktur_csv_' . $filtr->process($_POST['data_od']) . '_' . $filtr->process($_POST['data_do']) . '.csv');
           //
        } else { 
           //
           header('Content-disposition: attachment; filename=zestawienie_faktur_csv_' . $filtr->process($_POST['data_wydruku_mc']) . '_' . $filtr->process($_POST['data_wydruku_rok']) . '.csv');
           //
      }      
