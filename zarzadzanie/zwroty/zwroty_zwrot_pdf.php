...
if ($prot->wyswietlStrone) {
  ...
  if ( isset($_GET['id_poz']) && (int)$_GET['id_poz'] > 0 ) {
      ...
      $tlumacz = $i18n->tlumacz( array('WYGLAD', 'KLIENCI', 'KLIENCI_PANEL', 'REKLAMACJE', 'PRODUKT') );
      ...
      $pdf->SetTitle($tlumacz['NAGLOWEK_ZGLOSZENIE_ZWROTU']);
      $pdf->SetSubject($tlumacz['NAGLOWEK_ZGLOSZENIE_ZWROTU']);
      $pdf->SetKeywords($tlumacz['NAGLOWEK_ZGLOSZENIE_ZWROTU']);
      ...
      $text = PDFZwrot::WydrukZwrotPDF($_GET['id_poz']);
      ...
      $pdf->Output('zwrot_'.$_GET['id_poz'].'_'.time().'.pdf', 'D');
