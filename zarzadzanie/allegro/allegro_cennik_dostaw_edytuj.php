...
if ($prot->wyswietlStrone) {
    ...
    if (isset($_POST['akcja']) && $_POST['akcja'] == 'zapisz') {
        ...
        $_GET['id_poz'] = $_POST['id'];

        unset($_POST['akcja']);
        $DaneDoWyslania = new stdClass();

        $DaneDoWyslania->id = $_POST['id'];
        ...
        $wynik = $AllegroRest->commandPut('sale/shipping-rates/'.$DaneDoWyslania->id, $DaneDoWyslania );

        if ( isset($wynik->errors) && count($wynik->errors) > 0 ) {
            foreach ( $wynik->errors as $blad ) {
                $KomunikatBledow .= $blad->code . ' - ' . $blad->userMessage . '<br />';
            }
            //$AllegroRest->PokazBlad('Błąd', $KomunikatBledow, 'allegro_cennik_dostaw.php', 'false');
        } else {
            Funkcje::PrzekierowanieURL('allegro_cennik_dostaw.php?id_poz='.$_POST["id"]);
        }
    }

    $DaneWejsciowe = $_GET['id_poz']; 
    $WlaczoneWysylki = $AllegroRest->commandRequest('sale/shipping-rates', $DaneWejsciowe, '' );
    ...
    <div id="cont">
          ...
          <form action="allegro/allegro_cennik_dostaw_edytuj.php" method="post" id="allegroForm" class="cmxform">          

          <div class="poleForm">
            <div class="naglowek">Edycja danych cennika dostaw - data ostatniej modyfikacji <?php echo $WlaczoneWysylki->lastModified; ?></div>
                ...
                <div class="pozycja_edytowana">
                    ...
                    <p>
                      ...
                      <input type="text" name="name" size="53" value="<?php echo ( isset($_POST['name']) ? $_POST['name'] : $WlaczoneWysylki->name ); ?>" id="name" />
                    </p>
                  ...
                  <div class="ObramowanieForm" style="margin-top:10px;">
                    ...
                    foreach ( $DostepneWysylki->deliveryMethods as $Wysylka ) {
                        ...
                        echo '<tr class="PozycjaAllegroForm KoszyWysylkiAllegro">';
                            ...
                            foreach ( $WlaczoneWysylki->rates as $WysylkaBiezaca ) {

                                if ( $WysylkaBiezaca->deliveryMethod->id == $Wysylka->id ) {

                                    if ( isset($Wysylka->id) && isset($_POST) && array_key_exists($Wysylka->id, $_POST) ) {
                                        $wartoscPierwsza = $_POST[$Wysylka->id]['first-item'];
                                    } else {
                                        $wartoscPierwsza = $WysylkaBiezaca->firstItemRate->amount;
                                    }

                                    if ( isset($Wysylka->id) && isset($_POST) && array_key_exists($Wysylka->id, $_POST) ) {
                                        $wartoscDruga = $_POST[$Wysylka->id]['second-item'];
                                    } else {
                                        $wartoscDruga = $WysylkaBiezaca->nextItemRate->amount;
                                    }

                                    if ( isset($Wysylka->id) && isset($_POST) && array_key_exists($Wysylka->id, $_POST) ) {
                                        $wartoscIlosc = $_POST[$Wysylka->id]['max-quantity'];
                                    } else {
                                        $wartoscIlosc = $WysylkaBiezaca->maxQuantityPerPackage;
                                    }
