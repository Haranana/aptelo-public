...
if ($prot->wyswietlStrone) {
    ...
    $zapytanie = "SELECT p.products_id, 
              ...
              RIGHT JOIN products_related_products s ON p.products_id = s.prp_products_id_master " . $warunki_szukania . ' GROUP BY p.products_id '; 
    ...
    if (isset($_GET['parametr'])) {
        ...
        if ($ile_pozycji > 0) {
            ...
            $tablica_naglowek[] = array('Ilość produktów powiazanych','center');  
            ...
            while ($info = $sql->fetch_assoc()) {
                  ...
                  $zapytanie_powiazane = "select count(*) as ile_powiazanych from products_related_products where prp_products_id_master = '".$info['products_id']."'";
                  $sql_powiazane = $db->open_query($zapytanie_powiazane);
                  $infs = $sql_powiazane->fetch_assoc();
                  //
                  $tablica[] = array('<b>'.$infs['ile_powiazanych'].'</b>','center');
                  //
                  $db->close_query($sql_powiazane);
                  //
                  unset($infs);
                  ...
                  $tekst .= '<a class="TipChmurka" href="powiazane/powiazane_edytuj.php'.$zmienne_do_przekazania.'"><b>Edytuj</b><img src="obrazki/edytuj.png" alt="Edytuj" /></a>'; 
                  $tekst .= '<a class="TipChmurka" href="powiazane/powiazane_usun.php'.$zmienne_do_przekazania.'"><b>Skasuj</b><img src="obrazki/kasuj.png" alt="Skasuj" /></a>';
    ...
    if (!isset($_GET['parametr'])) { 
        ...
        $(document).ready(function() {
          $.AutoUzupelnienie( 'szukaj', 'Podpowiedzi', 'ajax/autouzupelnienie_produkty.php?typ=powiazane', 50, 350 );
        });
        </script> 
        ...
        <div id="caly_listing">
            ...
            <div id="naglowek_cont">Produkty powiązane</div>
            
            <div id="wyszukaj">
                <form action="powiazane/powiazane.php" method="post" id="poForm" class="cmxform"> 
                ...
                if ( Listing::wylaczFiltr(basename($_SERVER['SCRIPT_NAME'])) == true ) {
                  echo '<div id="wyszukaj_ikona"><a href="powiazane/powiazane.php?filtr=nie"><img src="obrazki/reset_szukaj.png" alt="Anuluj wyszukiwanie" /></a></div>';
                }
                ?>            
            ...
            <form action="powiazane/powiazane_akcja.php" method="post" class="cmxform">
            
            <div id="sortowanie">
            
                <span>Sortowanie: </span>
                
                <a id="sort_a1" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a1">brak</a>
                <a id="sort_a17" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a17">nazwy rosnąco</a>
                <a id="sort_a2" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a2">nazwy malejąco</a>
                <a id="sort_a7" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a7">nr katalogowy rosnąco</a>
                <a id="sort_a8" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a8">nr katalogowy malejąco</a> 
                <a id="sort_a9" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a9">cena rosnąco</a>
                <a id="sort_a10" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a10">cena malejąco</a>             
                <a id="sort_a3" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a3">aktywne</a>
                <a id="sort_a4" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a4">nieaktywne</a> 
                <a id="sort_a13" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a13">ID malejąco</a>
                <a id="sort_a14" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a14">ID rosnąco</a>
                <a id="sort_a15" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a15">sortowanie malejąco</a>
                <a id="sort_a16" class="sortowanie" href="powiazane/powiazane.php?sort=sort_a16">sortowanie rosnąco</a>
            ...
            <div id="PozycjeIkon">
                <div>
                    <a class="dodaj" href="powiazane/powiazane_dodaj.php">dodaj nową pozycję</a>
                </div>         
            </div>
                ...
                <div class="GlownyListingKategorie">
                    ...
                    <div class="OknoKategoriiKontener">
                        ...
                        for ($w = 0, $c = count($tablica_kat); $w < $c; $w++) {
                            ...
                            //
                            echo '<tr>
                                    <td class="lfp"><a href="powiazane/powiazane.php?kategoria_id='.$tablica_kat[$w]['id'].'" '.$style.'>'.$tablica_kat[$w]['text'].'</a></td>
                                    <td class="rgp" '.(($podkategorie) ? 'id="img_'.$tablica_kat[$w]['id'].'"' : '').'>'.(($podkategorie) ? '<img src="obrazki/rozwin.png" alt="Rozwiń" onclick="podkat(\''.$tablica_kat[$w]['id'].'\',\'\',\'\',\'\',\'powiazane\')" />' : '').'</td>
                                  </tr>
                        ...
                        if (isset($_GET['kategoria_id']) && (int)$_GET['kategoria_id'] > 0) {
                            ...
                            if (count($cSciezka) > 1) {
                                ...
                                <script>          
                                podkat('<?php echo $analiza_sciezki; ?>', '<?php echo $cSciezka[count($cSciezka)-1]; ?>','','','powiazane');
                                </script>
                    ...
                    <div id="akcja" class="AkcjaOdstep">
                        ...
                        <div id="akc">
                            ...
                            <select name="akcja_dolna" id="akcja_dolna">
                                ...
                                <option value="1">usuń z zaznaczonych produkty powiązane</option>
                            </select>
                        </div>
            ...
            <?php Listing::pokazAjax('powiazane/powiazane.php', $zapytanie, $ile_licznika, $ile_pozycji, 'products_id', ILOSC_WYNIKOW_NA_STRONIE, ADMIN_DOMYSLNE_SORTOWANIE); ?>
            </script>              
