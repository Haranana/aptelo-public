...
if ($prot->wyswietlStrone) {
    ...
    if (isset($_POST['akcja']) && $_POST['akcja'] == 'zapisz') {
        //
        $licz = (int)$filtr->process($_POST["ilosc_generowana"]);
        //
        if ($licz > 0) {
            //
            for ($pref = 0; $pref < $licz; $pref++) {
                //
                $los = los((int)$filtr->process($_POST["liczba_znakow"]));
                //
                // trzeba sprawdzic czy takiego kodu juz nie ma w bazie
                $zapytanie = "select coupons_name from coupons where coupons_name = '" . $filtr->process($_POST["pref"]) . $los . "'";
                ...
                if ((int)$db->ile_rekordow($sql) == 0) {            
                    ...
                    $pola = array(
                            ...
                            array('coupons_name',$filtr->process($_POST["pref"]) . $los),
                    ...
                    if ($_POST['warunek'] == 'kategoria_producent' && isset($_POST['id_producent']) && count($_POST['id_producent']) > 0 && isset($_POST['id_kat']) && count($_POST['id_kat']) > 0) {
                        $pola[] = array('coupons_exclusion','kategorie_producenci'); 
                        //
                        $tablica_producent = $_POST['id_producent'];
                    ...
                    if ( isset($_POST['tylko_do_produktu']) && $_POST['tylko_do_produktu'] == 'tak' && isset($_POST['id_produkt_powiazany']) && count($_POST['id_produkt_powiazany']) > 0) {
                        ...
                        $lista = substr((string)$lista, 0, -1);
                        //
                        $pola[] = array('coupons_products_only_id',$lista); 
                        unset($tablica_produkt, $lista);
                    } else {
    ...
    <div id="cont">
          ...
          <form action="kupony/kupony_dodaj_seria_losowa.php" method="post" id="kuponyForm" class="cmxform">          
          ...
          <div class="poleForm">
            ...
            <div class="pozycja_edytowana">
                ...
                <input type="hidden" name="akcja" value="zapisz" />
                    ...
                    <p>
                      <label class="required" for="liczba_znakow">Z ilu znaków ma mieć kupon ?</label>
                      <input type="text" name="liczba_znakow" id="liczba_znakow" value="" size="10" /><em class="TipIkona"><b>Wartość liczbowa powyżej 0, ilość znaków bez prefixu - tylko losowego kodu</b></em>
                    </p>

                    <p>
                      <label class="required" for="ilosc_generowana">Ile kuponów wygenerować ?</label>
                      <input type="text" name="ilosc_generowana" id="ilosc_generowana" value="" size="10" /><em class="TipIkona"><b>Wartość liczbowa powyżej 0</b></em>
                    </p>                    
                    ...
                    <div class="RamkaWarunki">
                        ...
                        <p>
                          ...
                          <input type="radio" value="1" name="promocja" id="promocja_tak" checked="checked" /><label class="OpisFor" for="promocja_tak">tak<em class="TipIkona"><b>Czy kuponem mają być objęte produkty promocyjne / wyprzedaży oraz przypisane jako produkt dnia ?</b></em></label>
                          <input type="radio" value="0" name="promocja" id="promocja_nie" /><label class="OpisFor" for="promocja_nie">nie<em class="TipIkona"><b>Czy kuponem mają być objęte produkty promocyjne / wyprzedaży oraz przypisane jako produkt dnia ?</b></em></label>
                        </p>                     
                    ...
                    <div style="padding:12px;">
                        <div class="ostrzezenie odlegloscRwd">Jeżeli w bazie będzie istniał kupon o generowanym numerze kupon nie zostanie dodany.</div>
                    </div>                       
