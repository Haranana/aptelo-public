...
if ($prot->wyswietlStrone) {
    ...
    if (isset($_POST['akcja']) && $_POST['akcja'] == 'zapisz') {
        ...
        $pola = array(
                ...
                array('discount_categories_id',(int)$_POST['id_kat']),
                array('discount_manufacturers_id',(int)$_POST['id_producent']));

        if (isset($_POST["tryb_rabat"]) && (int)$_POST["tryb_rabat"] == 1) {
            $pola[] = array('discount_groups_id',',' . implode(',', (array)$_POST["grupa_klientow"]) . ',');
        }                
        //	
        $db->update_query('discount_categories_manufacturers' , $pola, " discount_id = '".(int)$_POST["id"]."'");	

        unset($pola);
        //
        Funkcje::PrzekierowanieURL('rabaty_kategorii_producenta.php?id_poz='.(int)$_POST["id"]);
    }
    ...
    <div id="cont">
          ...
          $(document).ready(function() {
            ...
            $('.pkproducent td').find('input').click( function() {
          ...
          <form action="klienci/rabaty_kategorii_producenta_edytuj.php" method="post" id="eForm" class="cmxform">          
          ...
          <div class="poleForm">
            ...
            $zapytanie = "select * from discount_categories_manufacturers where discount_id = '" . (int)$_GET['id_poz'] . "'";
            ...
            if ((int)$db->ile_rekordow($sql) > 0) {
                ...
                <div class="pozycja_edytowana">
                    ...
                    <div class="info_content">
                        ...
                        <div class="WybieranieKategoriiProducenta" id="rpk">
                            ...
                            <div id="drzewo_kategorii" style="margin:0px">
                                ...
                                for ($w = 0, $c = count($tablica_kat); $w < $c; $w++) {
                                    ...
                                    //
                                    $check = '';
                                    if ( $tablica_kat[$w]['id'] == $info['discount_categories_id'] ) {
                                        $check = 'checked="checked"';
                                    }
                                    //                                
                                    echo '<tr>
                                            <td class="lfp"><input type="radio" value="'.$tablica_kat[$w]['id'].'" name="id_kat" id="id_kat_' . $tablica_kat[$w]['id'] . '" '.$check.' /> <label class="OpisFor" for="id_kat_' . $tablica_kat[$w]['id'] . '">'.$tablica_kat[$w]['text'].(($tablica_kat[$w]['status'] == 0) ? '<div class="wylKat TipChmurka"><b>Kategoria jest nieaktywna</b></div>' : '').'</label></td>
                                ...
                                //
                                $sciezka = Kategorie::SciezkaKategoriiId($info['discount_categories_id']);
                                ...
                                if (count($cSciezka) > 1) {
                                    ...
                                    <script>          
                                    podkat('<?php echo $analiza_sciezki; ?>', '<?php echo $cSciezka[count($cSciezka)-1]; ?>','radio','<?php echo $info['discount_categories_id']; ?>');
                                    </script>
                        ...
                        <p>
                          <label>Producent do jakiego będzie przypisany rabat:</label>
                        </p>
                        
                        <div class="WybieranieKategoriiProducenta">
                        
                            <div id="drzewo" class="WyborProducentaRabat" style="margin:0px">
                              ...
                              if (count($Prd) > 0) {
                                  //
                                  echo '<table class="pkc pkproducent">';
                                  //
                                  for ($b = 0, $c = count($Prd); $b < $c; $b++) {
                                      echo '<tr>                                
                                              <td class="lfp">
                                                  <input type="radio" value="'.$Prd[$b]['id'].'" name="id_producent" id="id_producent_' . $Prd[$b]['id'] . '" ' . (($Prd[$b]['id'] == $info['discount_manufacturers_id']) ? 'checked="checked"' : '') . '/> <label class="OpisFor" for="id_producent_' . $Prd[$b]['id'] . '">'.$Prd[$b]['text'].'</label>
                                              </td>                                
                                            </tr>';
                                  }
                                  echo '</table>';
                ...
                <div class="przyciski_dolne">
                  ...
                  <button type="button" class="przyciskNon" onclick="cofnij('rabaty_kategorii_producenta','<?php echo Funkcje::Zwroc_Wybrane_Get(array('id_poz')); ?>','klienci');">Powrót</button>   
                </div>
