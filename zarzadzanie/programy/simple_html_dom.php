	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
	{
		...
		foreach($nodes as $node) {
			...
			if ($pass && $id !== '' && isset($node->attr['id'])) {
				...
				$node_id = explode(' ', trim((string)$node->attr['id']))[0];
				...
				&& !empty($attributes)) {
					foreach($attributes as $a) {
						...
						if ($lowercase) {
							$check = $this->match(
								$att_expr,
								strtolower((string)$att_val),
								strtolower((string)$nodeKeyValue),
								$att_case_sensitivity
							);
						} else {
							$check = $this->match(
	...
	protected function match($exp, $pattern, $value, $case_sensitivity)
	{
		...
		switch ($exp) {
			...
			case '~=':
				 ...
				 */
				return in_array($pattern, explode(' ', trim((string)$value)), true);
		}
		return false;
	}
			...
			if($m[4] !== '') {
				...
				foreach($attributes as $att) {
					...
					$m[4][] = array(
						...
						(isset($att[4])) ? strtolower((string)$att[4]) : '', // Case-Sensitivity
					);
				}
			}
...
class simple_html_dom
{
	...
	protected function parse_charset()
	{
		...
		// it instead.
		if ((strtolower((string)$charset) == strtolower('ISO-8859-1'))
			|| (strtolower((string)$charset) == strtolower('Latin1'))
			|| (strtolower((string)$charset) == strtolower('Latin-1'))) {
	...
	protected function read_tag()
	{
		...
		if ($this->char === '/') {
			...
			$parent_lower = strtolower((string)$this->parent->tag);
			$tag_lower = strtolower((string)$tag);
			...
			if ($parent_lower !== $tag_lower) {
				...
				if (isset($this->optional_closing_tags[$parent_lower])
					...
					while (($this->parent->parent)
						&& strtolower((string)$this->parent->tag) !== $tag_lower
					){
						$this->parent = $this->parent->parent;
					}

					// If we don't have a match add current tag as text node
					if (strtolower((string)$this->parent->tag) !== $tag_lower) {
					...
					&& strtolower((string)$this->parent->parent->tag) === $tag_lower
		...
		if (isset($this->optional_closing_tags[$tag_lower])) {
			...
			while (isset($this->optional_closing_tags[$tag_lower][strtolower((string)$this->parent->tag)])) {
				$this->parent->_[HDOM_INFO_END] = 0;
				$this->parent = $this->parent->parent;
			}
		...
		do {
			...
			if ($name !== '/' && $name !== '') { // this is a attribute name
				...
				if ($this->lowercase) { $name = strtolower((string)$name); }
		...
		if ($this->copy_until_char('>') === '/') {
			...
			// reset parent
			if (!isset($this->self_closing_tags[strtolower((string)$node->tag)])) {
				$this->parent = $node;
			}
		}
	...
	function search_noise($text)
	{
		...
		foreach($this->noise as $noiseElement) {
			if (strpos((string)$noiseElement, $text) !== false) {
				return $noiseElement;
			}
		}
	}
