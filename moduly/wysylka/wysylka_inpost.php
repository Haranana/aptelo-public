...
if(!class_exists('wysylka_inpost')) {
  class wysylka_inpost {
    ...
    function __construct( $parametry = array(), $kraj = '', $idProduktu = '', $WagaProduktu = '', $CenaProduktu = '', $WysylkiProduktu = '', $GabarytProduktu = '', $KosztWysylkiProduktu = '0', $WykluczonaDarmowaWysylka = 'nie', $ProduktWPromocji = '0' ) {
      global $zamowienie, $Tlumaczenie, $token_inpost;
        ...
        if ( isset($this->paramatery['parametry']['WYSYLKA_TOKEN_INPOST']) ) {
            $token_inpost                 = $this->paramatery['parametry']['WYSYLKA_TOKEN_INPOST'];
        } else {
            $token_inpost                 = '';
        }
    ...
    public static function potwierdzenie( $koszyk = false ) {
        global $Tlumaczenie, $token_inpost;
        ...
        $tekst .= '<script>
                        ...
                        $(document).ready(function() {
                            ...
                            $("#WidgetButtonPaczkomaty").click(function(e) {
                            e.preventDefault();
        ...
        if ( $koszyk == true ) { 
             $tekst .= '<div class="WybranyPunktMapyKoszyk" id="WybranyPaczkomat" style="border:0;' . ((!isset($_SESSION['rodzajDostawyKoszyk']['wysylka_inpost']['opis'])) ? 'display:none' : '') . '">'.(isset($_SESSION['rodzajDostawyKoszyk']['wysylka_inpost']['opis']) ? $_SESSION['rodzajDostawyKoszyk']['wysylka_inpost']['opis'] : '').'</div>';
        }
        
        $tekst .= '<span class="przycisk" id="WidgetButtonPaczkomaty" style="font-style:normal">'.$Tlumaczenie['INPOST_WYBIERZ_PACZKOMAT'].'</span><br />';

        if ( $koszyk == false ) {
             $tekst .= '<input type="text" size="40" class="WybranyPunktMapyPaczkomaty" wysylka="wysylka_inpost" id="paczkomat" ' . ((!isset($_SESSION['rodzajDostawy']['opis'])) ? 'style="display:none"' : '') . ' value="'.(isset($_SESSION['rodzajDostawy']['opis']) ? $_SESSION['rodzajDostawy']['opis'] : '').'" name="lokalizacjaRuch" readonly="readonly" />';
             $tekst .= '<input type="hidden" id="ShippingDestinationCode" value="" name="ShippingDestinationCode" />';
             $tekst .= '<input type="hidden" id="RodzajPaczkomat" value="" />';
        }
