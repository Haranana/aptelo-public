...
if(!class_exists('wysylka_dpdpickup')) {
  class wysylka_dpdpickup {
    ...
    function __construct( $parametry = array(), $kraj = '', $idProduktu = '', $WagaProduktu = '', $CenaProduktu = '', $WysylkiProduktu = '', $GabarytProduktu = '', $KosztWysylkiProduktu = '0', $WykluczonaDarmowaWysylka = 'nie', $ProduktWPromocji = '0' ) {
      global $zamowienie, $Tlumaczenie, $token_dpd;
        ...
        if ( isset($this->paramatery['parametry']['WYSYLKA_API_KEY_DPDPICKUP']) ) {
            $token_dpd                 = $this->paramatery['parametry']['WYSYLKA_API_KEY_DPDPICKUP'];
        } else {
            $token_dpd                 = '';
        }
    ...
    public static function potwierdzenie( $koszyk = false ) {
        ...
        $tekst = '<script>
                  ...
                  $(document).ready(function() {
                     ...
                     $("#WidgetButtonDPD").click(function(e) {
                        e.preventDefault();
        ...
        if ( $koszyk == true ) { 
             $tekst .= '<div class="WybranyPunktMapyKoszyk" id="WybranyPunktDPD" style="border:0;' . ((!isset($_SESSION['rodzajDostawyKoszyk']['wysylka_dpdpickup']['opis'])) ? 'display:none' : '') . '">'.(isset($_SESSION['rodzajDostawyKoszyk']['wysylka_dpdpickup']['opis']) ? $_SESSION['rodzajDostawyKoszyk']['wysylka_dpdpickup']['opis'] : '').'</div>';
        }
        
        $tekst .= '<span class="przycisk" id="WidgetButtonDPD" style="font-style:normal">'.$Tlumaczenie['POCZTAPOLSKA_WYBIERZ_PUNKT'].'</span><br />';

        if ( $koszyk == false ) {          
             $tekst .= '<input type="text" size="40" class="WybranyPunktMapyDPD" id="punktDPD" ' . ((!isset($_SESSION['rodzajDostawy']['opis'])) ? 'style="display:none"' : '') . ' value="'.(isset($_SESSION['rodzajDostawy']['opis']) ? $_SESSION['rodzajDostawy']['opis'] : '').'" name="lokalizacjaRuch" readonly="readonly" id="wybor_ruchu" />';
        ...
        $tekst .= '<style>
                      #WynikMapDPD { min-height:600px; width:80vw; }

                      #punktDPD { width:100%; padding:10px; margin:10px 0px 0px 0px; font-size:110%; font-weight:bold;
