...
if(!class_exists('wysylka_dhlparcelshop')) {
  class wysylka_dhlparcelshop {
    ...
    public static function potwierdzenie( $koszyk = false ) {
        ...
        $tekst = '<script>
                  ...
                  $(document).ready(function() {
                     $("#WidgetButtonDhl").click(function(e) {
                        e.preventDefault();
        ...
        if ( $koszyk == true ) { 
             $tekst .= '<div class="WybranyPunktMapyKoszyk" id="WybranyPunktDhl" style="border:0;' . ((!isset($_SESSION['rodzajDostawyKoszyk']['wysylka_dhlparcelshop']['opis'])) ? 'display:none' : '') . '">'.(isset($_SESSION['rodzajDostawyKoszyk']['wysylka_dhlparcelshop']['opis']) ? $_SESSION['rodzajDostawyKoszyk']['wysylka_dhlparcelshop']['opis'] : '').'</div>';
        }
        
        $tekst .= '<span class="przycisk" id="WidgetButtonDhl" style="font-style:normal">'.$Tlumaczenie['POCZTAPOLSKA_WYBIERZ_PUNKT'].'</span><br />';

        if ( $koszyk == false ) {          
             $tekst .= '<input type="text" size="40" class="WybranyPunktMapyDhl" id="punktDhl" ' . ((!isset($_SESSION['rodzajDostawy']['opis'])) ? 'style="display:none"' : '') . ' value="'.(isset($_SESSION['rodzajDostawy']['opis']) ? $_SESSION['rodzajDostawy']['opis'] : '').'" name="lokalizacjaRuch" readonly="readonly" id="wybor_ruchu" />';
        ...
        $tekst .= '<style>
                      #WynikMapDhl { height:80vh; width:80vw; }
                      #WynikMapDhl iframe { position:absolute; top:0px; left:0px; right:0px; bottom:0px; height:100%; width:100%; }
