...
if(!class_exists('wysylka_pocztapunkt')) {
  class wysylka_pocztapunkt {
    ...
    function przetwarzanie() {
      ...
      /*
      $url = 'https://mapa.ecommerce.poczta-polska.pl';
      $ch = curl_init($url);
    ...
    public static function potwierdzenie( $koszyk = false ) {
        ...
        $pobranie = 'var pobranie = false; ';
        if ( $koszyk == false ) {   
             if ( isset($_SESSION['rodzajPlatnosci']['platnosc_klasa']) && $_SESSION['rodzajPlatnosci']['platnosc_klasa'] == 'platnosc_standard' ) {
                 $pobranie = "var pobranie = true; ";
             }
        }
        
        if ( $koszyk == true ) { 
             $adres_ulica = "Warszawa";
        } else {
             $adres_ulica = $_SESSION['adresDostawy']['miasto'];
        }

        $zapytanie_punkty = "SELECT wartosc FROM modules_shipping_params WHERE kod = 'PUNKTY_ODBIORU_POCZTA' and modul_id = (SELECT id FROM modules_shipping WHERE klasa = 'wysylka_pocztapunkt')";
        $sql_punkty = $GLOBALS['db']->open_query($zapytanie_punkty);
        $info_punkty = $sql_punkty->fetch_assoc();  
         
        $jakie_punkty = $info_punkty['wartosc'];
        
        $GLOBALS['db']->close_query($sql_punkty);         
        unset($zapytanie_punkty, $info_punkty);           

        $tekst = '<script src="https://mapa.ecommerce.poczta-polska.pl/widget/scripts/ppwidget.js"></script>
                  <script>
                  $(document).ready(function() {
                     $("#WidgetButtonPoczta").click(function() {

                        $.post("inne/mapy_koszyk.php?tok=' . Sesje::Token() . '", { koszyk: ' . (($koszyk == true) ? '"tak"' : '"nie"') . ', adres: "' . $adres . '", pobranie: "' . $pobranie . '", ulica: "' . $adres_ulica . '", punkty: "' . $jakie_punkty . '", modul: "pocztaodbior" }, function(data) { 
                            $("#WynikMapPocztaOdbior").html(data);         
                        });
        ...
        $tekst .= '<div id="WynikMapPocztaOdbior"></div>';
        ...
        if ( $koszyk == true ) { 
             $tekst .= '<div class="WybranyPunktMapyKoszyk" id="WybranyPunktPoczta" style="border:0;' . ((!isset($_SESSION['rodzajDostawyKoszyk']['wysylka_pocztapunkt']['opis'])) ? 'display:none' : '') . '">'.(isset($_SESSION['rodzajDostawyKoszyk']['wysylka_pocztapunkt']['opis']) ? $_SESSION['rodzajDostawyKoszyk']['wysylka_pocztapunkt']['opis'] : '').'</div>';
        }
        
        $tekst .= '<span class="przycisk" id="WidgetButtonPoczta" style="font-style:normal">'.$Tlumaczenie['POCZTAPOLSKA_WYBIERZ_PUNKT'].'</span><br />';

        if ( $koszyk == false ) {
             $tekst .= '<input type="text" size="40" class="WybranyPunktMapyPoczta" id="punktPoczta" ' . ((!isset($_SESSION['rodzajDostawy']['opis'])) ? 'style="display:none"' : '') . ' value="'.(isset($_SESSION['rodzajDostawy']['opis']) ? $_SESSION['rodzajDostawy']['opis'] : '').'" name="lokalizacjaRuch" readonly="readonly" />';
             ...
             $tekst .= '<div id="punktOpis" class="OpisInfoItalic WybranyPunktMapyPoczta" style="border:0;' . ((!isset($_SESSION['rodzajDostawy']['opispunkt'])) ? 'display:none' : '') . '">'.(isset($_SESSION['rodzajDostawy']['opispunkt']) ? str_replace('#', '<br />', (string)$_SESSION['rodzajDostawy']['opispunkt']) : '').'</div>';
        }
