...
if(!class_exists('wysylka_inpost_weekend')) {
  class wysylka_inpost_weekend {
    ...
    function __construct( $parametry = array(), $kraj = '', $idProduktu = '', $WagaProduktu = '', $CenaProduktu = '', $WysylkiProduktu = '', $GabarytProduktu = '', $KosztWysylkiProduktu = '0', $WykluczonaDarmowaWysylka = 'nie', $ProduktWPromocji = '0' ) {
      global $zamowienie, $Tlumaczenie, $token_inpost_weekend;
        ...
        $this->dostepna        = $this->paramatery['parametry']['WYSYLKA_DOSTEPNOSC'];

        $token_inpost_weekend                 = '';

        $zapytanie = "SELECT modul_id, kod, wartosc FROM modules_shipping_params WHERE kod = 'WYSYLKA_TOKEN_INPOST'";
        $sql = $GLOBALS['db']->open_query($zapytanie);

        while ($info = $sql->fetch_assoc()) {
            if ( $info['kod'] == 'WYSYLKA_TOKEN_INPOST' ) {
                $token_inpost_weekend = $info['wartosc'];
            }
        }
    ...
    function przetwarzanie() {
      ...
      if ( isset($this->dostepna) && $this->dostepna != '' ) {
          $Okres = array();
          $Okres = explode(':', (string)$this->dostepna);

          $OkresStart = explode('-', (string)$Okres['0']);
          $OkresKoniec = explode('-', (string)$Okres['1']);

          if ( $OkresKoniec['0'] == '0' ) { $OkresKoniec['0'] = '7'; }

          $DzienBiezacy = date('w', time());
          $GodzinaBiezaca = date('H', time());

          if ( $DzienBiezacy >= $OkresStart['0'] && $DzienBiezacy <= $OkresKoniec['0'] ) {
            if ( $DzienBiezacy == $OkresStart['0'] && $GodzinaBiezaca < $OkresStart['1'] ) {
                return;
            }
            if ( $DzienBiezacy == $OkresKoniec['0'] && $GodzinaBiezaca > $OkresKoniec['1'] ) {
                return;
            }
          } else {
            return;
    ...
    public static function potwierdzenie( $koszyk = false ) {
        global $Tlumaczenie, $token_inpost_weekend;
        ...
        $tekst .= '<script>
                        ...
                        $(document).ready(function() {
                            ...
                            $("#WidgetButtonPaczkomatyWeekend").click(function() {
                            e.preventDefault();
                            ...
                            myModal = new jBox("Modal", {
                              ...
                              ajax: {
                                ...
                                data: {
                                  ...
                                  adres: "' . $adres . '",
                                  modul: "paczkomaty_weekend",
                                  token: "' . $token_inpost_weekend .'",
                                },
                                setContent: false,
        ...
        if ( $koszyk == true ) { 
             $tekst .= '<div class="WybranyPunktMapyKoszyk" id="WybranyPaczkomatWeekend" style="border:0;' . ((!isset($_SESSION['rodzajDostawyKoszyk']['wysylka_inpost_weekend']['opis'])) ? 'display:none' : '') . '">'.(isset($_SESSION['rodzajDostawyKoszyk']['wysylka_inpost_weekend']['opis']) ? $_SESSION['rodzajDostawyKoszyk']['wysylka_inpost_weekend']['opis'] : '').'</div>';
        }
        
        $tekst .= '<span class="przycisk" id="WidgetButtonPaczkomatyWeekend" style="font-style:normal">'.$Tlumaczenie['INPOST_WYBIERZ_PACZKOMAT'].'</span><br />';

        if ( $koszyk == false ) {
             $tekst .= '<input type="text" size="40" class="WybranyPunktMapyPaczkomaty" wysylka="wysylka_inpost_weekend" id="paczkomatWeekend" ' . ((!isset($_SESSION['rodzajDostawy']['opis'])) ? 'style="display:none"' : '') . ' value="'.(isset($_SESSION['rodzajDostawy']['opis']) ? $_SESSION['rodzajDostawy']['opis'] : '').'" name="lokalizacjaRuch" readonly="readonly" />';
             $tekst .= '<input type="hidden" id="ShippingDestinationCodeWeekend" value="" name="ShippingDestinationCode" />';
             $tekst .= '<input type="hidden" id="RodzajPaczkomatWeekend" value="" />';
        }
        
        $tekst .= '</div></div>';
        
        $tekst .= '<style>
                      #WidokMapWysylkaPaczkomaty { height:80vh; width:80vw; }
                      
                      @media only screen and (max-width:480px) {
                        #WidokMapWysylkaPaczkomaty { height:80vh; width:90vw; }
                      }
                      #paczkomatWeekend { width:100%; padding:10px; margin:10px 0px 0px 0px; font-size:110%; font-weight:bold;
      ...
      switch ($RodzajOplaty) {
          ...
          case '5':
            ...
            if ( isset($_SESSION['koszyk']) ) {
                ...
                foreach ( $_SESSION['koszyk'] as $rekord ) {
                    //
                    if ( isset($rekord['inpost_gabaryt']) && ($rekord['inpost_gabaryt'] == 'x' || $rekord['inpost_gabaryt'] == 'd') ) {
            ...
            if ( $this->produktId != '' ) {
                ...
                if ( $wszystkieInpost == true && $tablica_kosztow_inpost[0] == 'GABARYT' ) {
                     ...
                     if ( isset($_SESSION['koszyk']) && count($_SESSION['koszyk']) > 0 ) {
                        ...
                        if ($calkowity_rozmiar <= 1) {
                            ...
                            $num_C = intdiv($calkowity_rozmiar, 4);
                            $reszta = $calkowity_rozmiar % 4;
                ...
                unset($koszt_gabaryt_a, $koszt_gabaryt_b, $koszt_gabaryt_c);
