...
if(!class_exists('wysylka_inpost_international')) {
  class wysylka_inpost_international {
    ...
    function __construct( $parametry = array(), $kraj = '', $idProduktu = '', $WagaProduktu = '', $CenaProduktu = '', $WysylkiProduktu = '', $GabarytProduktu = '', $KosztWysylkiProduktu = '0', $WykluczonaDarmowaWysylka = 'nie', $ProduktWPromocji = '0' ) {
      global $zamowienie, $Tlumaczenie, $token_inpost, $dostepne_kraje;
        ...
        $token_inpost          = $this->paramatery['parametry']['WYSYLKA_TOKEN_INPOST_INTERNATIONAL'];
        $dostepne_kraje        = $this->paramatery['parametry']['WYSYLKA_KRAJE_DOSTAWY'];
    ...
    function przetwarzanie() {
      global $token_inpost, $dostepne_kraje;
    ...
    public static function potwierdzenie( $koszyk = false ) {
        global $Tlumaczenie, $token_inpost, $dostepne_kraje;
        ...
        $tekst .= '<script>
                        ...
                        $(document).ready(function() {
                            ...
                            $("#WidgetButtonPaczkomatyInternational").click(function(e) {
                            e.preventDefault();
                            ...
                            myModal = new jBox("Modal", {
                              ...
                              ajax: {
                                ...
                                data: {
                                  ...
                                  adres: "' . $adres . '",
                                  modul: "paczkomaty_international",
                                  token: "' . $token_inpost .'",
                                  dostepne_kraje: "'.$dostepne_kraje.'"
                                },
                                setContent: false,
        ...
        if ( $koszyk == true ) { 
             $tekst .= '<div class="WybranyPunktMapyKoszyk" id="WybranyPaczkomatInternational" style="border:0;' . ((!isset($_SESSION['rodzajDostawyKoszyk']['wysylka_inpost_international']['opis'])) ? 'display:none' : '') . '">'.(isset($_SESSION['rodzajDostawyKoszyk']['wysylka_inpost_international']['opis']) ? $_SESSION['rodzajDostawyKoszyk']['wysylka_inpost_international']['opis'] : '').'</div>';
        }
        
        $tekst .= '<span class="przycisk" id="WidgetButtonPaczkomatyInternational" style="font-style:normal">'.$Tlumaczenie['INPOST_WYBIERZ_PACZKOMAT'].'</span><br />';

        if ( $koszyk == false ) {
             $tekst .= '<input type="text" size="40" class="WybranyPunktMapyPaczkomaty" wysylka="wysylka_inpost_international" id="paczkomatInternational" ' . ((!isset($_SESSION['rodzajDostawy']['opis'])) ? 'style="display:none"' : '') . ' value="'.(isset($_SESSION['rodzajDostawy']['opis']) ? $_SESSION['rodzajDostawy']['opis'] : '').'" name="lokalizacjaRuch" readonly="readonly" />';
             $tekst .= '<input type="hidden" id="ShippingDestinationCodeInternational" value="" name="ShippingDestinationCode" />';
             $tekst .= '<input type="hidden" id="RodzajPaczkomatInternational" value="" />';
        }
        ...
        $tekst .= '<style>
                      ...
                      #paczkomatInternational { width:100%; padding:10px; margin:10px 0px 0px 0px; font-size:110%; font-weight:bold;
