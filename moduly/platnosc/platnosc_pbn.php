...
if(!class_exists('platnosc_pbn')) {
  class platnosc_pbn {
    ...
    function podsumowanie( $id_zamowienia = 0 ) {
        ...
        $time = FunkcjeWlasnePHP::my_strtotime($zamowienie->info['data_zamowienia']);
        ...
        $accname                        = trim((string)$this->paramatery['parametry']['PLATNOSC_PBN_NAZWA_SPRZEDAWCY']) . '^NM^' . trim((string)$this->paramatery['parametry']['PLATNOSC_PBN_KOD_POCZTOWY']) . '^ZP^' . trim((string)$this->paramatery['parametry']['PLATNOSC_PBN_MIASTO']) . '^CI^' . trim((string)$this->paramatery['parametry']['PLATNOSC_PBN_ULICA']) . '^ST^' . trim((string)$this->paramatery['parametry']['PLATNOSC_PBN_PANSTWO']) . '^CT^';

        $parameters['id_client']        = '<id_client>' . preg_replace("/[^A-Za-z0-9]/", "", (string)$this->paramatery['parametry']['PLATNOSC_PBN_ID']) . '</id_client>';
        $parameters['id_trans']         = '<id_trans>' . sprintf("%010s", (($id_zamowienia == 0) ? (int)$_SESSION['zamowienie_id'] : $id_zamowienia)) . '</id_trans>';
        ...
        $parameters['account']          = '<account>' . preg_replace("/[^0-9]/", "", (string)$this->paramatery['parametry']['PLATNOSC_PBN_NUMER_KONTA']) . '</account>';
        $parameters['accname']          = '<accname>' . $accname . '</accname>';
        $parameters['backpage']         = '<backpage>' . ( WLACZENIE_SSL == 'tak' ? ADRES_URL_SKLEPU_SSL : ADRES_URL_SKLEPU ) . '/platnosc_koniec.php?typ=pbn&status=OK&zamowienie_id=' . (($id_zamowienia == 0) ? (int)$_SESSION['zamowienie_id'] : $id_zamowienia) . '</backpage>';
        $parameters['backpagereject']   = '<backpagereject>' . ( WLACZENIE_SSL == 'tak' ? ADRES_URL_SKLEPU_SSL : ADRES_URL_SKLEPU ) . '/platnosc_koniec.php?typ=pbn&status=FAIL&zamowienie_id=' . (($id_zamowienia == 0) ? (int)$_SESSION['zamowienie_id'] : $id_zamowienia) . '</backpagereject>';
