...
if(!class_exists('platnosc_cashbill')) {
  class platnosc_cashbill {
    ...
    function podsumowanie( $id_zamowienia = 0 ) {
        ...
        if ( $id_zamowienia == 0 ) {     
            ...
            $sign = md5 ( $this->paramatery['parametry']['PLATNOSC_CASHBILL_ID'].$zamowienie->info['wartosc_zamowienia_val'].strtoupper((string)$zamowienie->info['waluta']).'Numer zamowienia: ' . (int)$_SESSION['zamowienie_id'].$_SESSION['domyslnyJezyk']['kod'].$_SESSION['zamowienie_id'].
                trim((string)$_SESSION['adresDostawy']['imie']).
                trim((string)$_SESSION['adresDostawy']['nazwisko']).
                $zamowienie->klient['adres_email'].
                $zamowienie->klient['telefon'].
                $zamowienie->platnik['ulica'].
                $zamowienie->platnik['miasto'].
                $zamowienie->platnik['kod_pocztowy'].
                Funkcje::kodISOKrajuDostawy($_SESSION['adresDostawy']['panstwo']).
                $this->paramatery['parametry']['PLATNOSC_CASHBILL_SECRET'] );
        
        } else {
          
            $PodzielImieNazwisko = explode(' ', (string)$zamowienie->klient['nazwa']);
          
            $sign = md5 ( $this->paramatery['parametry']['PLATNOSC_CASHBILL_ID'].$zamowienie->info['wartosc_zamowienia_val'].strtoupper((string)$zamowienie->info['waluta']).'Numer zamowienia: ' . (int)$id_zamowienia . $_SESSION['domyslnyJezyk']['kod'] . (int)$id_zamowienia .
                trim((string)$PodzielImieNazwisko[0]) .
                trim((string)$PodzielImieNazwisko[count($PodzielImieNazwisko)-1]) .
                $zamowienie->klient['adres_email'] .
                $zamowienie->klient['telefon'] .
                $zamowienie->platnik['ulica'] .
                $zamowienie->platnik['miasto'] .
                $zamowienie->platnik['kod_pocztowy'] .
                Funkcje::kodISOKrajuDostawy($zamowienie->platnik['kraj']).
                $this->paramatery['parametry']['PLATNOSC_CASHBILL_SECRET'] ); 
        ...
        $parameters['rodzaj_platnosci'] = 'cashbill';

        $parameters['service']          = $this->paramatery['parametry']['PLATNOSC_CASHBILL_ID'];
        $parameters['amount']           = $zamowienie->info['wartosc_zamowienia_val'];
        $parameters['desc']             = 'Numer zamowienia: ' . (($id_zamowienia == 0) ? (int)$_SESSION['zamowienie_id'] : $id_zamowienia);
        $parameters['lang']             = $_SESSION['domyslnyJezyk']['kod'];
        $parameters['currency']         = strtoupper((string)$zamowienie->info['waluta']);
        $parameters['userdata']         = (($id_zamowienia == 0) ? (int)$_SESSION['zamowienie_id'] : $id_zamowienia);

        $PodzielImieNazwisko = explode(' ', (string)$zamowienie->klient['nazwa']);  
        
        $parameters['forname']          = (($id_zamowienia == 0) ? trim((string)$_SESSION['adresDostawy']['imie']) : trim((string)$PodzielImieNazwisko[0]));
        $parameters['surname']          = (($id_zamowienia == 0) ? trim((string)$_SESSION['adresDostawy']['nazwisko']) : trim((string)$PodzielImieNazwisko[count($PodzielImieNazwisko)-1]));
        
        unset($PodzielImieNazwisko);
        
        $parameters['email']            = $zamowienie->klient['adres_email'];
        $parameters['tel']              = $zamowienie->klient['telefon'];
        ...
        //$parameters['URL']              = ( WLACZENIE_SSL == 'tak' ? ADRES_URL_SKLEPU_SSL : ADRES_URL_SKLEPU ) . '/platnosc_koniec.php?typ=cashbill&zamowienie_id=' . (int)$_SESSION['zamowienie_id'];
        //$parameters['URLC']             = ( WLACZENIE_SSL == 'tak' ? ADRES_URL_SKLEPU_SSL : ADRES_URL_SKLEPU ) . '/moduly/platnosc/raporty/cashbill/raport.php';
