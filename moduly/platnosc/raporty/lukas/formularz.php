...
function PowtorzPlatnosc( $parametry, $zamowienie_id ) {
    ...
    $zapytanie = "SELECT kod, wartosc FROM modules_payment_params WHERE kod LIKE '%_LUKAS_%'";
    ...
    if ( $termin > (PLATNOSC_LUKAS_TERMIN_PATNOSCI *24 * 3600) ) {
       $pola = array(
    ...
    if ( PLATNOSC_LUKAS_STATUS_ZAMOWIENIA_START > 0 ) {
        $status_zamowienia_id = PLATNOSC_LUKAS_STATUS_ZAMOWIENIA_START;
    } else {
    ...
    if ( $zamowienie->info['status_zamowienia'] == $status_zamowienia_id ) {
        ...
        $tekst = '<form action="https://ewniosek.credit-agricole.pl/eWniosek/simulator_u.jsp" method="post" name="payform" class="cmxform">
