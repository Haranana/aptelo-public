...
if(!class_exists('platnosc_ileasing')) {
  class platnosc_ileasing {
    ...
    function __construct( $parametry ) {
      global $zamowienie, $Tlumaczenie, $numer_sklepu;
        ...
        $numer_sklepu    = $this->paramatery['parametry']['PLATNOSC_ILEASING_PARTNERID'];
        ...
        $this->dostawca        = $this->paramatery['parametry']['PLATNOSC_DOSTAWCA'];
        unset($Tlumaczenie);
    ...
    function podsumowanie( $id_zamowienia = 0 ) {
      global $numer_sklepu;
        ...
        $parameters['rodzaj_platnosci']     = 'ileasing';

        $parameters['info']                     = (($id_zamowienia == 0) ? (int)$_SESSION['zamowienie_id'] : $id_zamowienia);
        $parameters['backurl']                  = ( WLACZENIE_SSL == 'tak' ? ADRES_URL_SKLEPU_SSL : ADRES_URL_SKLEPU ) . '/platnosc_koniec.php?typ=ileasing&status=OK&zamowienie_id='.(($id_zamowienia == 0) ? (int)$_SESSION['zamowienie_id'] : $id_zamowienia).'';
        $parameters['sklep']                    = ( WLACZENIE_SSL == 'tak' ? ADRES_URL_SKLEPU_SSL : ADRES_URL_SKLEPU ) . '/moduly/platnosc/raporty/ileasing/raport.php';

        $p = 0;

        foreach ( $zamowienie->produkty as $produkt ) {

            $parameters['przedmiot[' . $p . ']']          = $produkt['nazwa'];
            $parameters['stan[' . $p . ']']               = ( $produkt['stan'] != '' ? $produkt['stan'] : 'Nowy' );

            $parameters['dostawca[' . $p . ']']           = ( $this->dostawca != '' ? $this->dostawca : DANE_NAZWA_FIRMY_PELNA );
            $parameters['cena[' . $p . ']']               = $produkt['cena_koncowa_netto'];
            $parameters['ilosc[' . $p . ']']              = $produkt['ilosc'];
        ...
        $parameters['firma']                    = $zamowienie->platnik['firma'];
        $parameters['nip']                      = $zamowienie->platnik['nip'];
        $parameters['osoba']                    = $zamowienie->klient['nazwa'];
        ...
        $tekst .= '<form action="https://www.platformafinansowa.pl/wniosek/leasing/'.$numer_sklepu.'" method="post" enctype="multipart/form-data" name="payform" class="cmxform">
                   <div style="text-align:center;padding:5px;">
                      {__TLUMACZ:PRZEJDZ_DO_WNIOSKU_LEASINGOWEGO}:<br /><br />';
        $tekst .= $formularz;
        $tekst .= '   <input class="przyciskZaplac" type="submit" id="submitButton" value="{__TLUMACZ:PRZEJDZ_DO_WNIOSKU_LEASINGOWEGO}" />
                   </div>
                   </form>';
