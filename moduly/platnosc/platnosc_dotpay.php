...
if(!class_exists('platnosc_dotpay')) {
  class platnosc_dotpay {
    ...
    function podsumowanie( $id_zamowienia = 0 ) {
        ...
        $parameters['api_version']      = 'dev';
        $parameters['id']               = $this->paramatery['parametry']['PLATNOSC_DOTPAY_ID'];



        if ( $zamowienie->info['waluta'] == $_SESSION['domyslnaWaluta']['kod'] ) {
            ...
            $CenaPLN = $zamowienie->info['wartosc_zamowienia_val'] / $przelicznikWaluty;
            $CenaDocelowa = $CenaPLN * $_SESSION['domyslnaWaluta']['przelicznik'];

            $parameters['amount']           = number_format($CenaDocelowa, 2, ".", "");

        }

        //$parameters['amount']           = $zamowienie->info['wartosc_zamowienia_val'];
        $parameters['currency']         = $_SESSION['domyslnaWaluta']['kod'];
        $parameters['description']      = 'Numer zamowienia: ' . (($id_zamowienia == 0) ? (int)$_SESSION['zamowienie_id'] : $id_zamowienia);
        ...
        $parameters['URL']              = ( WLACZENIE_SSL == 'tak' ? ADRES_URL_SKLEPU_SSL : ADRES_URL_SKLEPU ) . '/platnosc_koniec.php?typ=dotpay&zamowienie_id=' . (($id_zamowienia == 0) ? (int)$_SESSION['zamowienie_id'] : $id_zamowienia);
        $parameters['URLC']             = ( WLACZENIE_SSL == 'tak' ? ADRES_URL_SKLEPU_SSL : ADRES_URL_SKLEPU ) . '/moduly/platnosc/raporty/dotpay/raport.php';
        
        $parameters['type']             = '3';
        $parameters['buttontext']       = $this->txtguzik;

        $parameters['control']          = $zamowienie->info['wartosc_zamowienia_val'];

        $PodzielImieNazwisko = explode(' ', (string)$zamowienie->klient['nazwa']);
        
        $parameters['firstname']       = (($id_zamowienia == 0) ? trim((string)$_SESSION['adresDostawy']['imie']) : $PodzielImieNazwisko[0]);
        $parameters['lastname']        = (($id_zamowienia == 0) ? trim((string)$_SESSION['adresDostawy']['nazwisko']) : $PodzielImieNazwisko[count($PodzielImieNazwisko)-1]);        
        ...
        $chk = $this->paramatery['parametry']['PLATNOSC_DOTPAY_PIN'] . $parameters['api_version'] . $parameters['lang'] . $parameters['id'] . $parameters['amount'] . $parameters['currency'] . $parameters['description'] . $parameters['control'] . $parameters['channel'] . $parameters['ch_lock'] . $parameters['onlinetransfer'] . $parameters['URL'] . $parameters['type'] . $parameters['buttontext'] . $parameters['URLC'] . $parameters['firstname'] .  $parameters['lastname'] . $parameters['email'] . $parameters['street'] . $parameters['city'] . $parameters['postcode'] . $parameters['phone'] . $parameters['country'];

        $ChkValue = hash('sha256',$chk);

        $parameters['chk']          = $ChkValue;
        ...
        $tekst .= '<form action="https://dproxy.przelewy24.pl/" method="post" name="payform" class="cmxform">
