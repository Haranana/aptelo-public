...
// plik
$WywolanyPlik = 'wyprzedaz';

include('start.php');

$LinkDoPrzenoszenia = Seo::link_SEO('wyprzedaz.php', '', 'inna');
...
// breadcrumb
$nawigacja->dodaj($GLOBALS['tlumacz']['NAGLOWEK_WYPRZEDAZ']);
$tpl->dodaj('__BREADCRUMB', $nawigacja->sciezka(' ' . $GLOBALS['tlumacz']['NAWIGACJA_SEPARATOR'] . ' '));

$zapytanie = Produkty::SqlWyprzedazZlozone($WarunkiFiltrowania, $Sortowanie);                              
//
...
if (POKAZUJ_FILTRY_CECH == 'tak') {
    $srodek->dodaj('__FILTRY_PO_CECHACH', Filtry::FiltrSelect( Filtry::FiltrCech('', 'wyprzedaz'), 'c' ));
} else {
...
if (POKAZUJ_FILTRY_DODATKOWE_POLA == 'tak') {
    $srodek->dodaj('__FILTRY_PO_DODATKOWYCH_POLACH', Filtry::FiltrSelect( Filtry::FiltrDodatkowePola('', 'wyprzedaz'), 'p' ));     
} else {
...
if (POKAZUJ_FILTRY_KATEGORIE == 'tak') {
    $srodek->dodaj('__FILTRY_KATEGORIA', Filtry::FiltrKategoriiSelect( '', 'wyprzedaz' ));
} else {
...
if (POKAZUJ_FILTRY_PRODUCENCI == 'tak') {
    $srodek->dodaj('__FILTRY_PRODUCENT', Filtry::FiltrProducentaSelect( '', 'wyprzedaz' ));
} else {
...
if (POKAZUJ_FILTRY_DOSTEPNOSCI == 'tak') {
    $srodek->dodaj('__FILTRY_DOSTEPNOSCI', Filtry::FiltrDostepnoscSelect( '', 'wyprzedaz' ));
} else {
...
if (POKAZUJ_FILTRY_CZAS_WYSYLKI == 'tak') {
    $srodek->dodaj('__FILTRY_CZAS_WYSYLKI', Filtry::FiltrCzasWysylkiSelect( '', 'wyprzedaz' ));
} else {
