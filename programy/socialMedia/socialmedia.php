...
try {
    ...
    if ( !defined('ADRES_URL_SKLEPU') ) {
         header("Location: /");
         exit();
    }
    
    HttpClient\Util::redirect(((defined('ADRES_URL_SKLEPU')) ? ADRES_URL_SKLEPU : '') . '/inne/socialmedia.php?social=' . ((isset($_GET['provider'])) ? $_GET['provider'] : '') . '&powrot=' . ((isset($_GET['powrot'])) ? $_GET['powrot'] : ''));    
} catch (Exception $e) {
    echo $e->getMessage();
}
