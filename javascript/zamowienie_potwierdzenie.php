...
$PlikCacheJs = 'cache/js/zamowienie_potwierdzenie_' . $_SESSION['domyslnyJezyk']['kod'] . '.jcs';
...
if (!file_exists($PlikCacheJs) || CACHE_JS == 'nie') {
    ...
    $kod .= file_get_contents('javascript/zamowienie_potwierdzenie.jcs');

    // tlumaczenia
    $i18n = new Translator($_SESSION['domyslnyJezyk']['id']);
    $tlumacz = $i18n->tlumacz( array('FORMULARZ','WYSYLKI') );
...
$kod = str_replace( '{__TOKEN_ZAMOWIENIE_POTWIERDZENIE}', (string)Sesje::Token(), (string)$kod );
$kod = str_replace( '{__TOKEN_ADRES_DOSTAWY}', (string)Sesje::Token(), (string)$kod );
