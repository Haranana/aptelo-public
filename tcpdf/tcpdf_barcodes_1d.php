...
class TCPDFBarcode {
	...
	public function getBarcodeSVG($w=2, $h=30, $color='black') {
		...
		//header('Content-Length: '.strlen((string)$code));
		echo $code;
	}
	...
	protected function barcode_i25($code, $checksum=false) {
		...
		// add start and stop codes
		$code = 'AA'.strtolower((string)$code).'ZA';
		...
		for ($i = 0; $i < $clen; $i = ($i + 2)) {
			...
			$seq = '';
			$chrlen = strlen((string)$chr[$char_bar]);
			for ($s = 0; $s < $chrlen; $s++){
				$seq .= $chr[$char_bar][$s] . $chr[$char_space][$s];
			}
			$seqlen = strlen((string)$seq);
			for ($j = 0; $j < $seqlen; ++$j) {
				if (($j % 2) == 0) {
					$t = true; // bar
				} else {
			...
			case 'A': { // MODE A
				...
				for ($i = 0; $i < $len; ++$i) {
					...
					if (($char_id >= 241) AND ($char_id <= 244)) {
						...
						$code_data[] = strpos((string)$keys_a, (string)$char);
					} else {
						return false;
					}
				}
			...
			case 'B': { // MODE B
				...
				for ($i = 0; $i < $len; ++$i) {
					...
					if (($char_id >= 241) AND ($char_id <= 244)) {
						...
						$code_data[] = strpos((string)$keys_b, (string)$char);
					} else {
						return false;
					}
				}
				...
				if (isset($numseq[1]) AND !empty($numseq[1])) {
					...
					foreach ($numseq[1] as $val) {
						...
						if ($offset > $end_offset) {
							...
							$sequence = array_merge($sequence, $this->get128ABsequence(substr((string)$code, (int)$end_offset, (int)($offset - $end_offset))));
						}
						// numeric sequence
						$slen = strlen((string)$val[0]);
						if (($slen % 2) != 0) {
							// the length must be even
							--$slen;
						}
						$sequence[] = array('C', substr((string)$code, (int)$offset, (int)$slen), $slen);
						$end_offset = $offset + $slen;
					}
					if ($end_offset < $len) {
						$sequence = array_merge($sequence, $this->get128ABsequence(substr((string)$code, (int)$end_offset)));
					}
				} else {
	...
	protected function get128ABsequence($code) {
		...
		if (isset($numseq[1]) AND !empty($numseq[1])) {
			...
			foreach ($numseq[1] as $val) {
				...
				if ($offset > $end_offset) {
					// B sequence
					$sequence[] = array('B', substr((string)$code, (int)$end_offset, (int)($offset - $end_offset)), ($offset - $end_offset));
				}
				// A sequence
				$slen = strlen((string)$val[0]);
				$sequence[] = array('A', substr((string)$code, (int)$offset, (int)$slen), $slen);
				$end_offset = $offset + $slen;
			}
			if ($end_offset < $len) {
				$sequence[] = array('B', substr((string)$code, (int)$end_offset), ($len - $end_offset));
			}
		} else {
			// only B sequence
	...
	protected function barcode_eanupc($code, $len=13) {
		...
		if ($upce) {
			...
			$tmp = substr((string)$code, 4, 3);
			if (($tmp == '000') OR ($tmp == '100') OR ($tmp == '200')) {
				// manufacturer code ends in 000, 100, or 200
				$upce_code = substr((string)$code, 2, 2).substr((string)$code, 9, 3).substr((string)$code, 4, 1);
			} else {
				$tmp = substr((string)$code, 5, 2);
				if ($tmp == '00') {
					// manufacturer code ends in 00
					$upce_code = substr((string)$code, 2, 3).substr((string)$code, 10, 2).'3';
				} else {
					$tmp = substr((string)$code, 6, 1);
					if ($tmp == '0') {
						// manufacturer code ends in 0
						$upce_code = substr((string)$code, 2, 4).substr((string)$code, 11, 1).'4';
					} else {
						// manufacturer code does not end in zero
						$upce_code = substr((string)$code, 2, 5).substr((string)$code, 11, 1);
					}
				}
			}
		}
	...
	protected function barcode_codabar($code) {
		...
		$w = 0;
		$seq = '';
		$code = 'A'.strtoupper((string)$code).'A';
		$len = strlen((string)$code);
		for ($i = 0; $i < $len; ++$i) {
			if (!isset($chr[$code[$i]])) {
				return false;
	...
	protected function barcode_imb($code) {
		...
		switch (strlen((string)$routing_code)) {
			case 0: {
				$binary_code = 0;
				break;
			}
		...
		$binary_code .= substr((string)$tracking_number, 2, 18);
		...
		$binary_code_arr = substr((string)$binary_code_arr, 0, -1);
		$binary_code_arr = explode("\r", (string)$binary_code_arr);
		// calculate frame check sequence
		$fcs = $this->imb_crc11fcs($binary_code_arr);
		// exclude first 2 bits from first byte
		$first_byte = sprintf('%2s', dechex((hexdec($binary_code_arr[0]) << 2) >> 2));
		$binary_code_102bit = $first_byte.substr((string)$binary_code, 2);
	...
	protected function barcode_imb_pre($code) {
		...
		$characters = str_split(strtolower((string)$code), 1);
		// build bars
		$k = 0;
		$bararray = array('code' => $code, 'maxw' => 0, 'maxh' => 3, 'bcode' => array());
		for ($i = 0; $i < 65; ++$i) {
	...
	public function dec_to_hex($number) {
		...
		while($number > 0) {
			if($number == 0) {
				...
				array_push($hex, strtoupper((string)dechex(bcmod($number, '16'))));
				$number = bcdiv($number, '16', 0);
			}
		}
		$hex = array_reverse($hex);
		return implode((array)$hex);
	}
