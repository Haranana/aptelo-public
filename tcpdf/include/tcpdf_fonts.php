...
class TCPDF_FONTS {
	...
	public static function addTTFfont($fontfile, $fonttype='', $enc='', $flags=32, $outpath='', $platid=3, $encid=1, $addcbbox=false, $link=false) {
		...
		if (!isset($font_path_parts['filename'])) {
			$font_path_parts['filename'] = substr($font_path_parts['basename'], 0, -(strlen((string)$font_path_parts['extension']) + 1));
		}
		$font_name = strtolower((string)$font_path_parts['filename']);
		$font_name = preg_replace('/[^a-z0-9_]/', '', $font_name);
		$search  = array('bold', 'oblique', 'italic', 'regular');
		$replace = array('b', 'i', 'i', '');
		$font_name = str_replace($search, $replace, (string)$font_name);
		if (empty($font_name)) {
			// set generic name
			$font_name = 'tcpdffont';
		}
		...
		$fmetric['originalsize'] = strlen((string)$font);
		...
		if ($fmetric['type'] == 'Type1') {
			...
			$fmetric['bbox'] = trim((string)$matches[1]);
			$bv = explode(' ', (string)$fmetric['bbox']);
			...
			$fmetric['Leading'] = 0;
			// get charstring data
			$eplain = substr($eplain, (strpos((string)$eplain, '/CharStrings') + 1));
	...
	public static function _getTrueTypeFontSubset($font, $subsetchars) {
		...
		$table['loca']['length'] = strlen((string)$loca);
		...
		$table['glyf']['length'] = strlen((string)$glyf);
		...
		$checkSumAdjustment = 0xB1B0AFBA - self::_getTTFtableChecksum($font, strlen((string)$font));
