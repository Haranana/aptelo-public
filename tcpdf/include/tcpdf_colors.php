...
class TCPDF_COLORS {
	...
	public static function convertHTMLColorToDec($hcolor, &$spotc='', $defcol=array('R'=>128,'G'=>128,'B'=>128)) {
		...
		} elseif ((substr($color, 0, 4) != 'cmyk') AND (substr($color, 0, 3) != 'rgb') AND (($dotpos = strpos((string)$color, '.')) !== false)) {
		...
		if (strlen((string)$color) == 0) {
			return $defcol;
		}
		// RGB ARRAY
		if (substr($color, 0, 3) == 'rgb') {
			$codes = substr($color, 4);
			$codes = str_replace(')', '', (string)$codes);
			$returncolor = explode(',', (string)$codes);
