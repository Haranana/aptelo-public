...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function getImageFileType($imgfile, $iminfo=array()) {
		...
		if (isset($iminfo['mime']) AND !empty($iminfo['mime'])) {
			...
			if ((count($mime) > 1) AND ($mime[0] == 'image') AND (!empty($mime[1]))) {
				$type = strtolower(trim((string)$mime[1]));
			}
		}
		if (empty($type)) {
            $type = strtolower(trim(pathinfo(parse_url((string)$imgfile, PHP_URL_PATH), PATHINFO_EXTENSION)));
		}
		if ($type == 'jpg') {
			$type = 'jpeg';
		}
	...
	public static function _parsejpeg($file) {
		...
		$icc = array();
		$offset = 0;
		while (($pos = strpos((string)$data, "ICC_PROFILE\0", $offset)) !== false) {
			...
			$icc[($msn - 1)] = substr((string)$data, (int)($pos + 14), (int)$length);
