...
class PDF417 {
	...
	protected function getInputSequences($code) {
		...
		$numseq[1][] = array('', strlen((string)$code));
		$offset = 0;
		foreach($numseq[1] as $seq) {
			$seqlen = strlen((string)$seq[0]);
			if ($seq[1] > 0) {
				// extract text sequence before the number sequence
				$prevseq = substr((string)$code, (int)$offset, (int)($seq[1] - $offset));
				$textseq = array();
				// get text sequences
				preg_match_all('/([\x09\x0a\x0d\x20-\x7e]{5,})/', $prevseq, $textseq, PREG_OFFSET_CAPTURE);
				$textseq[1][] = array('', strlen((string)$prevseq));
				$txtoffset = 0;
				foreach($textseq[1] as $txtseq) {
					$txtseqlen = strlen((string)$txtseq[0]);
					if ($txtseq[1] > 0) {
						// extract byte sequence before the text sequence
						$prevtxtseq = substr((string)$prevseq, (int)$txtoffset, (int)($txtseq[1] - $txtoffset));
						if (strlen((string)$prevtxtseq) > 0) {
							// add BYTE sequence
							if ((strlen((string)$prevtxtseq) == 1) AND ((count($sequence_array) > 0) AND ($sequence_array[(count($sequence_array) - 1)][0] == 900))) {
								$sequence_array[] = array(913, $prevtxtseq);
							} elseif ((strlen((string)$prevtxtseq) % 6) == 0) {
	...
	protected function getCompaction($mode, $code, $addmode=true) {
		...
		switch($mode) {
			...
			case 924: { // Byte Compaction mode latch
				while (($codelen = strlen((string)$code)) > 0) {
					if ($codelen > 6) {
						$rest = substr((string)$code, 6);
						$code = substr((string)$code, 0, 6);
						$sublen = 6;
					} else {
						$rest = '';
						$sublen = strlen((string)$code);
					}
					if ($sublen == 6) {
			...
			case 902: { // Numeric Compaction mode latch
				while (($codelen = strlen((string)$code)) > 0) {
					if ($codelen > 44) {
						$rest = substr((string)$code, 44);
						$code = substr((string)$code, 0, 44);
					} else {
						$rest = '';
					}
					$t = '1'.$code;
