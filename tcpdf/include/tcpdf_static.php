...
class TCPDF_STATIC {
	 ...
	 * @return int Returns 1 otherwise.
	 * @since 4.6.025 (2009-08-17)
	 * @public static
	 */
			...
			switch ($position) {
				case 'start': {
					if (strpos((string)$border, 'B') !== false) {
						// remove bottom line
						$newkey = str_replace('B', '', (string)$border);
						if (strlen((string)$newkey) > 0) {
							$brd[$newkey] = $style;
						}
						unset($brd[$border]);
					}
				...
				case 'middle': {
					...
					if (strpos((string)$border, 'T') !== false) {
						// remove bottom line
						$newkey = str_replace('T', '', (string)$border);
						if (strlen((string)$newkey) > 0) {
							$brd[$newkey] = $style;
						}
						unset($brd[$border]);
	...
	public static function empty_string($str) {
		return (is_null($str) OR (is_string($str) AND (strlen((string)$str) == 0)));
	}
	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function replacePageNumAliases($page, $replace, $diff=0) {
		foreach ($replace as $rep) {
			foreach ($rep[3] as $a) {
				if (strpos((string)$page, $a) !== false) {
					$page = str_replace($a, $rep[0], (string)$page);
					$diff += ($rep[2] - $rep[1]);
				}
			}
		}
	...
	public static function _AES($key, $text) {
		...
		$padding = 16 - (strlen((string)$text) % 16);
	...
	public static function _RC4($key, $text, &$last_enc_key, &$last_enc_key_c) {
		if ($last_enc_key != $key) {
			$k = str_repeat($key, (int) ((256 / strlen((string)$key)) + 1));
			$rc4 = range(0, 255);
			$j = 0;
			for ($i = 0; $i < 256; ++$i) {
				$t = $rc4[$i];
	...
	public static function fixHTMLCode($html, $default_css, $tagvs, $tidy_options, &$tagvspaces) {
		...
		$html = str_replace('<br>', '<br />', (string)$html);
					...
					switch ($attrib[0]) {
						...
						case '[': { // attribute
							...
							if (preg_match('/\[([a-zA-Z0-9]*)[\s]*([\~\^\$\*\|\=]*)[\s]*["]?([^"\]]*)["]?\]/i', $attrib, $attrmatch) > 0) {
								$att = strtolower((string)$attrmatch[1]);
								$val = $attrmatch[3];
								if (isset($dom[$key]['attribute'][$att])) {
									switch ($attrmatch[2]) {
										...
										case '^=': {
											if ($val == substr($dom[$key]['attribute'][$att], 0, strlen((string)$val))) {
												$valid = true;
											}
											break;
										}
										case '$=': {
											if ($val == substr($dom[$key]['attribute'][$att], -strlen((string)$val))) {
												$valid = true;
											}
											break;
										}
										case '*=': {
											if (strpos((string)$dom[$key]['attribute'][$att], $val) !== false) {
												$valid = true;
											}
											break;
										}
	...
	public static function getTagStyleFromCSSarray($css) {
		...
		foreach ($css as $style) {
			...
			foreach ($csscmds as $cmd) {
				if (!empty($cmd)) {
					$pos = strpos((string)$cmd, ':');
					if ($pos !== false) {
						$cmd = substr($cmd, 0, ($pos + 1));
						if (strpos((string)$tagstyle, $cmd) !== false) {
	...
	public static function revstrpos($haystack, $needle, $offset = 0) {
		$length = strlen((string)$haystack);
		$offset = ($offset > 0)?($length - $offset):abs($offset);
		$pos = strpos(strrev((string)$haystack), strrev((string)$needle), $offset);
		return ($pos === false)?false:($length - $pos - strlen((string)$needle));
	}
	...
	public static function fileGetContents($file) {
		$alt = array($file);
		//
		if ((strlen((string)$file) > 1)
		    && ($file[0] === '/')
		    && ($file[1] !== '/')
		    && !empty($_SERVER['DOCUMENT_ROOT'])
		    && ($_SERVER['DOCUMENT_ROOT'] !== '/')
		) {
		    $findroot = strpos((string)$file, (string)$_SERVER['DOCUMENT_ROOT']);
		...
		if (preg_match('%^//%', $url) && !empty($_SERVER['HTTP_HOST'])) {
			$url = $protocol.':'.str_replace(' ', '%20', (string)$url);
		}
		...
		if (preg_match('%^(https?)://%', $url)
			...
			if (empty($urldata['query'])) {
				...
				if (strpos((string)$url, (string)$host) === 0) {
				    // convert URL to full server path
				    $tmp = str_replace($host, $_SERVER['DOCUMENT_ROOT'], (string)$url);
	...
	public static function rfread($handle, $length) {
		...
		$rest = ($length - strlen((string)$data));
