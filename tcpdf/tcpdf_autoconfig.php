...
if ((!isset($_SERVER['DOCUMENT_ROOT'])) OR (empty($_SERVER['DOCUMENT_ROOT']))) {
	if(isset($_SERVER['SCRIPT_FILENAME'])) {
		$_SERVER['DOCUMENT_ROOT'] = str_replace( '\\', '/', substr($_SERVER['SCRIPT_FILENAME'], 0, 0-strlen((string)$_SERVER['PHP_SELF'])));
...
if (!defined('K_PATH_URL')) {
	...
	if (isset($_SERVER['HTTP_HOST']) AND (!empty($_SERVER['HTTP_HOST']))) {
		...
		$k_path_url .= str_replace( '\\', '/', substr(K_PATH_MAIN, (strlen((string)$_SERVER['DOCUMENT_ROOT']) - 1)));
	}
