...
class TCPDF {
	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			if ($this->getRTL()) {
				$header_x = $this->original_rMargin + ((is_numeric($headerdata['logo_width']) ? $headerdata['logo_width'] : 0) * 1.1);
			} else {
				$header_x = $this->original_lMargin + ((is_numeric($headerdata['logo_width']) ? $headerdata['logo_width'] : 0) * 1.1);
			}
			$cw = $this->w - $this->original_lMargin - $this->original_rMargin - ((is_numeric($headerdata['logo_width']) ? $headerdata['logo_width'] : 0) * 1.1);
	...
	public function setColor($type, $col1=0, $col2=-1, $col3=-1, $col4=-1, $ret=false, $name='') {
		...
		switch ($type) {
			case 'draw': {
				$pdfcolor .= strtoupper((string)$suffix);
				$this->DrawColor = $pdfcolor;
	...
	public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		$style = '';
		// underline
		if (strpos((string)$tempstyle, 'U') !== false) {
			$this->underline = true;
		} else {
			$this->underline = false;
		}
		// line-through (deleted)
		if (strpos((string)$tempstyle, 'D') !== false) {
			$this->linethrough = true;
		} else {
			$this->linethrough = false;
		}
		// overline
		if (strpos((string)$tempstyle, 'O') !== false) {
			$this->overline = true;
		} else {
			$this->overline = false;
		}
		// bold
		if (strpos((string)$tempstyle, 'B') !== false) {
			$style .= 'B';
		}
		// oblique
		if (strpos((string)$tempstyle, 'I') !== false) {
			$style .= 'I';
		}
		$bistyle = $style;
		...
		if (($type != 'core') AND $missing_style) {
			...
			$name .= $styles[$bistyle];
			// artificial bold
			if (strpos((string)$bistyle, 'B') !== false) {
				if (isset($desc['StemV'])) {
					// from normal to bold
			...
			// artificial italic
			if (strpos((string)$bistyle, 'I') !== false) {
	...
	protected function getCellCode($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
		...
		$s = '';
		// fill and borders
		if (is_string($border) AND (strlen((string)$border) == 4)) {
			// full border
			$border = 1;
		}
		if ($fill OR ($border == 1)) {
	...
	public function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if ($file[0] === '@') {
            ...
            } elseif (($imsize = @getimagesize($file)) === FALSE && strpos((string)$file, '__tcpdf_'.$this->file_id.'_img') === FALSE){
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			$byte_range = array();
			$byte_range[0] = 0;
			$byte_range[1] = strpos((string)$pdfdoc, TCPDF_STATIC::$byterange_string) + $byterange_string_len + 10;
			$byte_range[2] = $byte_range[1] + $this->signature_max_length + 2;
			$byte_range[3] = strlen((string)$pdfdoc) - $byte_range[2];
			$pdfdoc = substr($pdfdoc, 0, $byte_range[1]).substr($pdfdoc, $byte_range[2]);
			// replace the ByteRange
			$byterange = sprintf('/ByteRange[0 %u %u %u]', $byte_range[1], $byte_range[2], $byte_range[3]);
			$byterange .= str_repeat(' ', ($byterange_string_len - strlen((string)$byterange)));
			...
			$pdfdoc_length = strlen((string)$pdfdoc);
			...
			$signature = substr($signature, (strpos((string)$signature, "%%EOF\n\n------") + 13));
			$tmparr = explode("\n\n", $signature);
			$signature = $tmparr[1];
			// decode signature
			...
			$this->bufferlen = strlen((string)$this->buffer);
		}
		switch($dest) {
			case 'I': {
		...
		if ($destroyall AND !$preserve_objcopy && isset($this->file_id)) {
			...
			if ($handle = @opendir(K_PATH_CACHE)) {
				while ( false !== ( $file_name = readdir( $handle ) ) ) {
					if (strpos((string)$file_name, '__tcpdf_'.$this->file_id.'_') === 0) {
			...
			if (isset($this->imagekeys)) {
				foreach($this->imagekeys as $file) {
					if (strpos((string)$file, K_PATH_CACHE) === 0 && TCPDF_STATIC::file_exists($file)) {
						@unlink($file);
					}
				}
			}
	...
	protected function replaceRightShiftPageNumAliases($page, $aliases, $diff) {
		foreach ($aliases as $type => $alias) {
			foreach ($alias as $a) {
				...
				if (($pos = strpos((string)$page, (string)$a)) !== false) {
					// end of alias
	...
	protected function _putpages() {
		...
		for ($n = 1; $n <= $num_pages; ++$n) {
			...
			$pagelen = strlen((string)$temppage);
					...
					if (isset($pl['opt']['ap'])) {
						...
						if (is_array($pl['opt']['ap'])) {
							foreach ($pl['opt']['ap'] as $apmode => $apdef) {
								...
								$annots .= ' /'.strtoupper((string)$apmode);
								if (is_array($apdef)) {
									$annots .= ' <<';
	...
	protected function _putXMP() {
		...
		$xmp .= '</x:xmpmeta>'."\n";
		$xmp .= '<?xpacket end="w"?>';
		$out = '<< /Type /Metadata /Subtype /XML /Length '.strlen((string)$xmp).' >> stream'."\n".$xmp."\n".'endstream'."\n".'endobj';
		// restore previous isunicode value
		$this->isunicode = $prev_isunicode;
	...
	protected function _out($s) {
		if ($this->state == 2) {
			if ($this->inxobj) {
				...
				$this->footerpos[$this->page] += strlen((string)$s."\n");
			} else {
				// set page data
	...
	public function addHtmlLink($url, $name, $fill=false, $firstline=false, $color=null, $style=-1, $firstblock=false) {
		if (isset($url[1]) AND ($url[0] == '#') AND is_numeric($url[1])) {
			...
			if (isset($lnkdata[0]) ) {
				...
				if (isset($lnkdata[1]) AND (strlen((string)$lnkdata[1]) > 0)) {
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			...
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				$envelope_length = strlen((string)$envelope);
				...
				// extract signature
				$signature = substr($signature, strpos((string)$signature, 'Content-Disposition'));
				$tmparr = explode("\n\n", $signature);
	...
	public function setProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		if ($mode > 1) { // AES
			if (!extension_loaded('openssl')) {
				$this->Error('AES encryption requires openssl extension (http://www.php.net/manual/en/mcrypt.requirements.php).');
			}
	...
	public function cropMark($x, $y, $w, $h, $type='T,R,B,L', $color=array(100,100,100,100,'All')) {
		...
		$crops = explode(',', strtoupper((string)$type));
		// remove duplicates
	...
	public function ImageEps($file, $x=null, $y=null, $w=0, $h=0, $link='', $useBoundingBox=true, $align='', $palign='', $border=0, $fitonpage=false, $fixoutvals=false) {
		...
		if (count($regs) > 1) {
			...
			if (strpos((string)$version_str, 'Adobe Illustrator') !== false) {
		...
		$data = substr($data, $start);
		$end = strpos((string)$data, '%%PageTrailer');
		if ($end===false) {
			$end = strpos((string)$data, 'showpage');
		}
		if ($end) {
	...
	protected function getHtmlDomArray($html) {
		...
		$html = strip_tags((string)$html, '<marker><a><b><blockquote><body><br><dd><del><div><dl><dt><em><font><form><h1><h2><h3><h4><h5><h6><hr><i><img><input><label><li><ol><option><p><pre><s><select><small><span><strike><strong><sub><sup><table><tablehead><td><textarea><th><thead><tr><tt><u><ul>');
		...
		$offset = 0;
		while (($offset < strlen((string)$html)) AND ($pos = strpos((string)$html, '</pre>', $offset)) !== false) {
			...
			$offset = strlen((string)$html_a.$html_b);
		}
		$offset = 0;
		while (($offset < strlen((string)$html)) AND ($pos = strpos((string)$html, '</textarea>', $offset)) !== false) {
			$html_a = substr($html, 0, $offset);
		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				if ($element[0] == '/') {
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						for ($i = ($dom[$key]['parent'] + 1); $i < $key; ++$i) {
							$dom[($dom[$key]['parent'])]['content'] .= stripslashes((string)$a[$dom[$i]['elkey']]);
						}
						$key = $i;
						// mark nested tables
				...
				$dom[$key]['value'] = stripslashes((string)$this->unhtmlentities($element));
			}
			++$elkey;
			++$key;
		}
				...
				if (isset($startlinex)) {
					...
					if ((isset($plalign) AND ((($plalign == 'C') OR ($plalign == 'J') OR (($plalign == 'R') AND (!$this->rtl)) OR (($plalign == 'L') AND ($this->rtl)))))) {
						...
						if ($this->isRTLTextDir()) { // RTL
							...
							$pos1 = TCPDF_STATIC::revstrpos((string)$pmid, '[(');
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(TCPDF_STATIC::revstrpos((string)$pmid, '[('.chr(0).chr(32)));
									$spacelen = 2;
								} else {
									$pos2 = intval(TCPDF_STATIC::revstrpos((string)$pmid, '[('.chr(32)));
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
									...
									} elseif ($pos1 == strpos((string)$pmid, '[(')) {
										$no = 1;
									}
								}
							}
							...
							if ($pos1 > 0) {
								$pos1 = intval($pos1);
								if ($this->isUnicodeFont()) {
									$pos2 = intval(TCPDF_STATIC::revstrpos((string)$pmid, chr(0).chr(32).')]')) + 2;
									$spacelen = 2;
								} else {
									$pos2 = intval(TCPDF_STATIC::revstrpos((string)$pmid, chr(32).')]')) + 1;
									$spacelen = 1;
								}
								if ($pos1 == $pos2) {
						...
						if ($plalign == 'C') {
							...
							if (preg_match_all('/\[\(([^\)]*)\)\]/x', $pmidtemp, $lnstring, PREG_PATTERN_ORDER)) {
								...
								while (preg_match('/([0-9\.\+\-]*)[\s](Td|cm|m|l|c|re)[\s]/x', $pmid, $strpiece, PREG_OFFSET_CAPTURE, $offset) == 1) {
									...
									$stroffset = strpos((string)$pmid, '[(', $offset);
									if (($stroffset !== false) AND ($stroffset <= $strpiece[2][1])) {
										// set offset to the end of string section
										$offset = strpos((string)$pmid, ')]', $stroffset);
										while (($offset !== false) AND ($pmid[($offset - 1)] == '\\')) {
											$offset = strpos((string)$pmid, ')]', ($offset + 1));
										}
										if ($offset === false) {
									...
									$offset = $strpiece[2][1] + strlen((string)$strpiece[2][0]);
									$epsposend = strpos((string)$pmid, $this->epsmarker.'Q', $offset);
									if ($epsposend !== null) {
										$epsposend += strlen((string)$this->epsmarker.'Q');
										$epsposbeg = strpos((string)$pmid, 'q'.$this->epsmarker, $offset);
										if ($epsposbeg === null) {
											$epsposbeg = strpos((string)$pmid, 'q'.$this->epsmarker, ($prev_epsposbeg - 6));
								...
								if ($this->isUnicodeFont()) {
									...
									$endlinepos = strlen((string)$pstart."\n".$pmid."\n");
								} else {
			...
			if ($dom[$key]['tag']) {
				...
				if ($newline) {
					if (!$this->premode) {
						...
						$postlen = strlen((string)$dom[$key]['value']);
			...
			case 'img': {
				...
				if ($imgsrc[0] === '@') {
					...
					if (($imgsrc[0] === '/') AND !empty($_SERVER['DOCUMENT_ROOT']) AND ($_SERVER['DOCUMENT_ROOT'] != '/')) {
						// fix image path
						$findroot = strpos((string)$imgsrc, $_SERVER['DOCUMENT_ROOT']);
			...
			case 'select': {
				...
				if (isset($tag['attribute']['opt']) AND !TCPDF_STATIC::empty_string($tag['attribute']['opt'])) {
					...
					foreach ($options as $val) {
						if (strpos((string)$val, '#!TaB!#') !== false) {
			...
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					foreach ($pnalias['u'] as $u) {
						$sfill = str_repeat($filler, max(0, (strlen((string)$u) - strlen((string)$nu.' '))));
						if ($this->rtl) {
					...
					foreach ($pnalias['a'] as $a) {
						$sfill = str_repeat($filler, max(0, (strlen((string)$a) - strlen((string)$na.' '))));
						if ($this->rtl) {
							$nr = $na.' '.$sfill;
						} else {
							$nr = $sfill.' '.$na;
		...
		if (!TCPDF_STATIC::empty_string($page)) {
			for ($p = $page_first; $p <= $page_last; ++$p) {
				...
				for ($n = 1; $n <= $maxpage; ++$n) {
					...
					foreach ($pnalias['u'] as $u) {
						if ($correct_align) {
							$sfill = str_repeat($filler, (strlen((string)$u) - strlen((string)$nu.' ')));
							if ($this->rtl) {
								$nr = $nu.TCPDF_FONTS::UTF8ToUTF16BE(' '.$sfill, false, $this->isunicode, $this->CurrentFont);
							} else {
								$nr = TCPDF_FONTS::UTF8ToUTF16BE($sfill.' ', false, $this->isunicode, $this->CurrentFont).$nu;
					...
					foreach ($pnalias['a'] as $a) {
						if ($correct_align) {
							$sfill = str_repeat($filler, (strlen((string)$a) - strlen((string)$na.' ')));
							if ($this->rtl) {
								$nr = $na.' '.$sfill;
							} else {
								$nr = $sfill.' '.$na;
			...
			switch (strtoupper((string)$cmd)) {
				...
				case 'S': { // shorthand/smooth curveto
					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if ((($ck + 1) % 4) == 0) {
							if (($key > 0) AND ((strtoupper((string)$paths[($key - 1)][1]) == 'C') OR (strtoupper((string)$paths[($key - 1)][1]) == 'S'))) {
								$x1 = (2 * $x) - $x2;
								$y1 = (2 * $y) - $y2;
							} else {
								$x1 = $x;
				...
				case 'T': { // shorthand/smooth quadratic Bezier curveto
					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if (($ck % 2) != 0) {
							if (($key > 0) AND ((strtoupper((string)$paths[($key - 1)][1]) == 'Q') OR (strtoupper((string)$paths[($key - 1)][1]) == 'T'))) {
								$x1 = (2 * $x) - $x1;
								$y1 = (2 * $y) - $y1;
							} else {
								$x1 = $x;
			...
			case 'image': {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						$img = '@'.base64_decode(substr($img, strlen((string)$m[0])));
					} else {
						// fix image path
						...
						if (($img[0] == '/') AND !empty($_SERVER['DOCUMENT_ROOT']) AND ($_SERVER['DOCUMENT_ROOT'] != '/')) {
							$findroot = strpos((string)$img, $_SERVER['DOCUMENT_ROOT']);
							if (($findroot === false) OR ($findroot > 1)) {
								if (substr($_SERVER['DOCUMENT_ROOT'], -1) == '/') {
